...
export default async function handler(req, res) {
  ...
  // Twilio sends payload as form urlencoded: From = "whatsapp:+91XXXXXXXXXX", Body = message text
  const fromRaw = req.body.From || '';
  const cleanPhone = fromRaw.replace('whatsapp:', '').replace(/[^0-9]/g, '');
  const text = (req.body.Body || '').trim();
  const upperText = text.toUpperCase();

  // Query user plan from Supabase
  ...
  // 1. Commute Confirmation Checkpoint ("1", "YES", "DONE")
  if (['1', 'YES', 'DONE', 'LEFT'].includes(upperText)) {
    const savedMins = 25;
    ...
    await fetch(`${supabaseUrl}/rest/v1/commute_feedback_log`, {
      ...
      body: JSON.stringify({
        ...
        scheduled_departure: plan ? plan.leave_home_time : 'Standard',
    ...
    replyText = `✅ *COMMUTE LOGGED!*\n\n` +
      `+${savedMins} mins saved by beating peak surge.\n` +
  ...
  // 3. Status Check
    ...
    replyText = `📊 *YOUR COMMUTE SCORECARD*\n\n` +
      ...
      `🔗 Dashboard: https://${req.headers.host || 'blr-commute.vercel.app'}/dashboard.html?phone=${cleanPhone}`;
  }
  // 4. On-Demand "GO TO <Place>"
    ...
    try {
      ...
      if (geoData.features && geoData.features.length) {
        ...
        replyText = `🔍 *ON-DEMAND: TO ${destTitle.toUpperCase()}*\n\n` +
          ...
          `🎯 Depart within 15m to avoid bottleneck surge.\n\n` +
          `🔗 Navigation: ${navLink}`;
      } else {
        replyText = `Could not locate "${queryDest}" in Bengaluru. Try area or landmark name.`;
      }
    } catch {
      replyText = `Routing service busy. Try again in a minute.`;
    }
  }
  // Default Help
  else {
    replyText = `🤖 *BLR Commute Radar*\n\n` +
      `• Reply *1* or *YES* when you leave\n` +
      `• Reply *STATUS* to see time saved\n` +
      `• Reply *WFH* to skip today's alerts\n` +
      `• Text *GO TO <place>* for instant routing`;
  }

  // Respond using Twilio TwiML XML
