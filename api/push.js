...
export default async function handler(req, res) {
  ...
  // 1. Fetch user subscription and alert history
  ...
  if (error || !commuter?.push_subscription) {
    return res.status(404).json({ error: 'No active device subscription found for this user.' });
  }

  // 2. Prevent duplicate alerts within a 4-hour window
  if (commuter.last_alert_sent_at) {
    const hoursSinceLast = (Date.now() - new Date(commuter.last_alert_sent_at).getTime()) / (1000 * 60 * 60);
    if (hoursSinceLast < 4) {
      return res.status(200).json({ status: 'SKIPPED_DUPLICATE', message: 'Alert already dispatched recently.' });
    }
  }
  ...
  try {
    ...
    return res.status(200).json({ success: true, message: 'Notification delivered' });
  } catch (err) {
